<?php
   ...
   if(isset($_POST['submit'])){
      ...
      if(!empty($name)){
         $update_name = $conn->prepare("UPDATE `customer` SET Name = ? WHERE CustomerID = ?");
      ...
      if(!empty($email)){
         ...
         if($select_email->rowCount() > 0){
            ...
            $update_email = $conn->prepare("UPDATE `customer` SET Email = ? WHERE CustomerID = ?");
      ...
      if(!empty($number)){
         ...
         if($select_number->rowCount() > 0){
            ...
            $update_number = $conn->prepare("UPDATE `customer` SET Number = ? WHERE CustomerID = ?");
            $update_number->execute([$number, $user_id]);
         }
      }

      if(!empty($address)){
         $select_address = $conn->prepare("SELECT * FROM `customer` WHERE Address = ?");
         $select_address->execute([$address]);
         if($select_address->rowCount() > 0){
            $message[] = 'address already taken!';
         } else {
            $update_address = $conn->prepare("UPDATE `customer` SET Address = ? WHERE CustomerID = ?");
            $update_address->execute([$address, $user_id]);
         }
      }
      
      $empty_pass = 'da39a3ee5e6b4b0d3255bfef95601890afd80709';
      $select_prev_pass = $conn->prepare("SELECT Password FROM `customer` WHERE CustomerID = ?");
      ...
      if($old_pass != $empty_pass){
         if($old_pass != $prev_pass){
            ...
            if($new_pass != $empty_pass){
               $update_pass = $conn->prepare("UPDATE `customer` SET Password = ? WHERE CustomerID = ?");
